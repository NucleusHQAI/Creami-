...
**How to work a task.** Read `CLAUDE.md`, then the spec listed under _Spec_, then do the task. Do not read the other documents — they are separated so that no task needs the whole picture in its head. Do not do work belonging to a later task, even if it seems small.
...
### Task 49 · Adapt online recipes

**Depends on:** 23, 24, 36, 39, 41, 47 · **Spec:** `docs/11-feature-online-recipe-adaptation.md`

Add a visible recipe-creation entry point and an adaptation assistant inside the new-recipe editor.
Extract Schema.org Recipe data through an authenticated, SSRF-protected Edge Function or accept a
pasted ingredient list. Suggest an existing base, map only to current UK library ingredients,
explain replacements, base coverage and unresolved lines, then populate the normal editable form.

Source website nutrition is never trusted: the existing macro engine calculates every adapted
recipe from the selected base, ingredient library and current settings. Save compact source
attribution after the recipe succeeds and display it safely on recipe detail.

**Done when:** URL and paste journeys produce an editable, explained draft with live app-calculated
macros; unsafe or unsupported sources fail clearly; and saved adaptations retain safe attribution.
